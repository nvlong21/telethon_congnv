        ...
        for k in CATEGORY_CRAWL.keys():
            ...
            for dict_from_chat in lst_dict_from_chat:
                ...
                async for message in client.iter_messages(intify("https://t.me/CoinMarketCap"), reverse=True, offset_id=offset):
                    ...
                    print('Working with: ',str(message.id), message.user_id, message.user)
...
def run(tele): 
    ...
    future = asyncio.ensure_future(tele.initial_crawl())
    loop.run_until_complete(future)
def run2(tele): 
    print("asda")
    loop = get_or_create_eventloop()
    future = asyncio.ensure_future( tele.initial_post())
    loop.run_until_complete(future)
...
asyncio.set_event_loop(loop)
# loop = get_or_create_eventloop()
# loop.create_task(initial_crawl())
