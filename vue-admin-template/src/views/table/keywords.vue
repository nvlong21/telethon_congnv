  ...
  <div class="modal-vue">
    ...
    <div v-if="open" class="modal">
      ...
      <el-form ref="form" :model="addKeyWords" label-width="120px">
        <el-form-item label="Key word category">
        ...
        <el-form-item label="Words">
          <el-input v-model="addKeyWords.list_keyword" />
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
    addCate() {
      ...
      fetch('http://localhost:8000/keywords', requestOptions).then(async response => {
        this.$router.go(this.$router.currentRoute)
      })
    },
    getKeyWorlds() {
      this.listLoading = true
      fetch('http://localhost:8000/keywords').then(async response => {
    ...
    getCategory(id) {
      fetch('http://localhost:8000/categories?cate_for=filter&type_id=3').then(async response => {
        const datas = await response.json()
